...
namespace Sistema_de_Controle_de_Viagens
{
    ...
    public partial class frm_transferencia : Form
    {
        ...
        public frm_transferencia(Form_cctII f)
        {
            InitializeComponent();
            frm = f;
        ...
        private void btnGravar_Click(object sender, EventArgs e)
        {
            ...
            if (txtPrefixo.Text != "" && int.Parse(txtPrefixo.Text) % 2 == frm.sequencia % 2)
            {
                ...
                if (banco_dados.status_consulta == true)
                {
                    ...
                    frm.odtAux = banco_dados.consulta(frm.timer1, "update Linha set atualizacao = (atualizacao + 1) where codigo = " + cmbLinha.SelectedValue, "linha");
        ...
        private void executaRecolher(string final_viagem, string local, string mensagem_trens, string mensagem_historico)
        {
            try
            {
                ...
                if (banco_dados.status_consulta == true)
                {
                    ...
                    if (banco_dados.status_consulta == true)
                    {
                        ...
                        banco_dados.consulta(frm.timer1, sql, "historico");
                        if (banco_dados.status_consulta== true)
                            if (frm.maquinista_saida != "" && (groupBox1.Text == "Recolher" || frm.codigo_local_vira == 5))
                            {
                                banco_dados.consulta(frm.timer1, "Insert into viras (codigo_local_vira, codigo_linha, hora_entrada, maquinista, sequencia) values "
            ...
            catch (Exception e)
            {
                ...
                banco_dados.consulta(frm.timer1, "Insert Into Logs (Log, data) Values ('Err CCT II recolher - " + e.Message + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')", "Log");
        ...
        private void executaAdicionar(string msg)
        {
            ...
            catch (Exception e)
            {
                ...
                banco_dados.consulta(frm.timer1, "Insert Into Logs (Log, data) Values ('Err CCT II adicionar - " + e.Message + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')", "Log");
                frm.timer1.Enabled = true;
            }
        ...
        public void verifica_mensagem(object sender, EventArgs e)
        {
            try
            {
                frm.odtAux = banco_dados.consulta(frm.timer1, "select mensagem from trens where trem = '" + cmbTrem.Text + "'", "trens");
        ...
        private void frm_transferencia_Load(object sender, EventArgs e)
        {
            ...
            cmbLinha.DataSource = banco_dados.consulta(frm.timer1, "Select * From linha where codigo <> " + frm.linha + "", "linha");
        }
        ...
        private void cmbLinha_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                frm.odtPar = banco_dados.consulta(frm.timer1, "Select max(sequencia) From Andamento Where Linha = " + cmbLinha.SelectedValue + " and Data = '" + frm.dia.Date.ToString("yyyy-MM-dd HH:mm:ss") + "' and (Sequencia mod 2) = " + define_sentido, "Andamento");
                if (frm.odtPar.Rows[0][0].ToString() == "")
                    frm.odtPar = banco_dados.consulta(frm.timer1, "Select max(sequencia) From historico Where Linha = " + cmbLinha.SelectedValue + " and Data = '" + frm.dia.Date.ToString("yyyy-MM-dd HH:mm:ss") + "' and (Sequencia mod 2) = " + define_sentido, "historico");
            ...
            catch (Exception ex)
            {
                ...
                banco_dados.consulta(frm.timer1, "Insert Into Logs (Log, data) Values ('Err CCT II prefixo impar Dbclik - " + ex.Message + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')", "Log");
                frm.timer1.Enabled = true;
            }
        }
