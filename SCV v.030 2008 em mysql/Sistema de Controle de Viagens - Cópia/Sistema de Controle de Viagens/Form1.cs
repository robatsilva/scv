...
namespace Sistema_de_Controle_de_Viagens
{
    public partial class Form1 : Form
    {
        ...
        int Tamanho;
        public bool status_consulta = false;
        string versao = "2";
        ...
        public DataTable consulta(string sql, string tabela)
        {
            try
            {
        ...
        private void btnGravar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    odtAux = consulta("select * from andamento where id = '" + cmbTrem.Text.ToUpper() + "'", "andamento");
                    if (odtAux.Rows.Count != 0)
                    {
        ...
        {
            Ok();
            table_linha = consulta("select pasta from linha where codigo = " + linha, "linha");
        ...
        {
            DataTable status_robo = consulta("select robo from linha where codigo  = " + linha, "linha");
            ...
            for (i = 0; i <= 1; i++)
            {
                andamento = consulta("select prefixo from andamento where linha = " + linha + " and sequencia % 2 = " + i.ToString() + " order by sequencia desc limit 1", "andamento");
                try
                {
        ...
        private void checkBox1_CheckedChanged_1(object sender, EventArgs e)
        {
            ...
            {
                timer2.Enabled = true;
                consulta("update linha set robo = 'sim' where codigo = " + linha, "linha");
            }
            else
            {
                timer2.Enabled = false;
                consulta("update linha set robo = 'não' where codigo = " + linha, "linha");
            }
        }
